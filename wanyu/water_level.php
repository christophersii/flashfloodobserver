...
$station_result = $conn->query("SELECT station.station_code, station.station_name, station.drainage_depth, sensor_reading.water_level, sensor_reading.reading_time FROM station 
...
while ($station_row = $station_result->fetch_assoc()) {
    ...
    if ($existing_result !== false && mysqli_num_rows($existing_result) > 0) {
        ...
        $conn->query("UPDATE setting SET station_name='$station_name', water_level='$water_level', reading_time='$reading_time', normal_level='$normal_level', warning_level='$warning_level', danger_level='$danger_level', level='$level' WHERE station_code='$station_id'");
    } else {
        // Insert a new row with the water level data, level and reading_time
        $conn->query("INSERT INTO setting (station_code, station_name, water_level, reading_time, normal_level, warning_level, danger_level, level) VALUES ('$station_id', '$station_name','$water_level', '$reading_time', '$normal_level', '$warning_level', '$danger_level', '$level')");
    }
